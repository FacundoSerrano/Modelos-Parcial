...
int eProgProy_mostrarProyectos(eProgramadorProyecto asignados[], int limite, eProyecto proyectos[], int id)
{
    ...
    printf("\nCantidad asignados %d", cantidadAsignados);
...
void eProgProy_demandante(eProgramadorProyecto asignados[], int limiteProgProy, eProyecto proyectos[])
{
    ...
    int mayor = 0;
    int indice;
    //printf("%d", eProgProy_buscarLugarLibre(asignados, limiteProgProy));
    for(i = 1; i <= eProgProy_buscarLugarLibre(asignados, limiteProgProy); i++)
    {
        printf("\nDato %d", eProgProy_mostrarProyectos(asignados, limiteProgProy, proyectos, i));
        cantidad = eProgProy_mostrarProyectos(asignados, limiteProgProy, proyectos, i);
        if(cantidad > mayor)
        {
            mayor = cantidad;
            indice = i;
